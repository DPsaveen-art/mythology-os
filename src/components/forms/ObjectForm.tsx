...
import { OBJECT_SUBTYPES } from '../../utils/formConstants'
import type { ObjectEntity } from '../../types'

interface ObjectFormProps {
  initial?: ObjectEntity | null
  onSubmit: (entity: ObjectEntity) => void
  onCancel: () => void
}
...
export function ObjectForm({ initial, onSubmit, onCancel }: ObjectFormProps) {
  ...
  useEffect(() => {
    if (initial) {
      ...
      setPowers((initial.powers ?? []).join(', '))
      setSymbolicMeaning(initial.symbolicMeaning ?? '')
    }
  }, [initial])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const entity: ObjectEntity = {
      ...
      powers: powers.split(/,\s*/).filter(Boolean).map((s) => s.trim()),
      symbolicMeaning: symbolicMeaning.trim() || undefined,
  ...
  return (
    <form onSubmit={handleSubmit} className="form">
      ...
      <div className="form-group">
        ...
        <input type="text" value={powers} onChange={(e) => setPowers(e.target.value)} placeholder="thunder, lightning" className="form-input" />
      </div>
      <div className="form-group">
        <label>Symbolic meaning</label>
        <input type="text" value={symbolicMeaning} onChange={(e) => setSymbolicMeaning(e.target.value)} className="form-input" />
      </div>
